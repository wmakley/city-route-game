...
func TestDeleteByIDDeletesNestedRecords(t *testing.T) {
	assert := assert.New(t)

	board := domain.Board{
		Name: "Test Board",
	}
	err := dbConn.Save(&board).Error
	assert.That(err).IsNil()
	...
	err = dbConn.Find(&cities, "board_id = ?", board.ID).Error
	assert.That(err).IsNil()
	assert.ThatInt(len(cities)).IsEqualTo(0)

	var spaces []domain.CitySpace
	err = dbConn.Find(&spaces, "city_id = ?", city.ID).Error
	assert.That(err).IsNil()
	assert.ThatInt(len(spaces)).IsEqualTo(0)

	var boards []domain.Board
	err = dbConn.Find(&boards, board.ID).Error
	assert.That(err).IsNil()
